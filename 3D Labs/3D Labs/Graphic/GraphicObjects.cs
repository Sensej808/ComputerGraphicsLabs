...
namespace _3D_Labs.Graphic.Objects
{
    ...
    public class Edge
    {
        ...
        public override int GetHashCode()
        {
            return HashCode.Combine(FirstPoint.X, FirstPoint.Y, FirstPoint.Z, SecondPoint.X, SecondPoint.Y, SecondPoint.Z);
        }
    ...
    public class Face
    {
        ...
        public List<float[]> texturePoints;
        ...
        public Face(List<Point> points, float accuracy = 0.001f)
        {
            ...
            */
            ...
            texturePoints = new List<float[]>();
            for (int i = 0; i < points.Count; i++)
            {
                texturePoints.Add(new float[2] { 0, 0 });
            }
    }
    ...
    public class Polyhedron
    {
        ...
        public void Update()
        {
            Dictionary<Point, float> pointX = new Dictionary<Point, float>();
            Dictionary<Point, float> pointY = new Dictionary<Point, float>();
            Dictionary<Point, float> pointZ = new Dictionary<Point, float>();
            Dictionary<Point, int> pointCnt = new Dictionary<Point, int>();
            for (int i = 0; i < points.Count; i++)
            {
                pointX.Add(points[i], 0);
                pointY.Add(points[i], 0);
                pointZ.Add(points[i], 0);
                pointCnt.Add(points[i], 0);
            }
            for (int i = 0; i < faces.Count; i++)
            {
                faces[i].Center();
                ...
                {
                    pointX[faces[i].points[j]] += faces[i].normalVector.X;
                    pointY[faces[i].points[j]] += faces[i].normalVector.Y;
                    pointZ[faces[i].points[j]] += faces[i].normalVector.Z;
                    pointCnt[faces[i].points[j]] += 1;
                }
            }
            for (int i = 0; i < points.Count; i++)
            {
                points[i].NormalVector[0] = pointX[points[i]] / pointCnt[points[i]];
                points[i].NormalVector[1] = pointY[points[i]] / pointCnt[points[i]];
                points[i].NormalVector[2] = pointZ[points[i]] / pointCnt[points[i]];
            }
        }
